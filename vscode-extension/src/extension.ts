...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (
    ...
    isTemplateFile(vscode.window.activeTextEditor.document)
  ) {
    ...
    vscode.languages.registerCompletionItemProvider(
      "dart-template",
      {
        async provideCompletionItems(document, position, token, context) {
          const templateItems = provideTemplatePlaceholderCompletions(
            document,
            position,
          );
          const dartItems = await shadowProvider.provideCompletionItems(
            document,
            position,
            token,
            context,
      ...
      "i",
      "e", // Trigger on logic start chars
    ),
  );
    ...
    vscode.DefinitionProvider
{
  ...
  private updateShadowFile(document: vscode.TextDocument) {
    ...
    const ignores =
      "// ignore_for_file: uri_does_not_exist, undefined_class, undefined_function, undefined_identifier, UNUSED_IMPORT, dead_code\n";

    // Replace ALL {{...}} content with block comments
    const sanitized =
      ignores + text.replace(/\{\{([\s\S]*?)\}\}/g, "/* {{$1}} */");
  ...
  private mapLocationFromShadow(location: vscode.Location): vscode.Location {
    if (location.uri.path.endsWith(".__analysis__.dart")) {
      const templatePath = location.uri.fsPath.replace(
        ".__analysis__.dart",
        "",
      );
  ...
  async provideDefinition(
    ...
    try {
      ...
      >("vscode.executeDefinitionProvider", shadowUri, shadowPos);
      ...
      if (Array.isArray(result)) {
        ...
        if ("targetUri" in result[0]) {
          ...
          return (result as vscode.Location[]).map((loc) =>
            this.mapLocationFromShadow(loc),
          );
        }
      } else {
...
function isTemplateFile(document: vscode.TextDocument): boolean {
  return (
    document.languageId === "dart-template" ||
    document.languageId === "shell-template"
  );
}
...
function validateDocument(document: vscode.TextDocument) {
  ...
  if (config.get("placeholderValidation", true)) {
    ...
    while ((match = PLACEHOLDER_REGEX.exec(text)) !== null) {
      ...
      if (
        /^(if\s+[a-zA-Z0-9_]+|else\s+if\s+[a-zA-Z0-9_]+|else|endif)(\s|$)/.test(
          content,
        )
      ) {
        continue;
...
function validateConditionalNesting(
  document: vscode.TextDocument,
): vscode.Diagnostic[] {
  const diagnostics: vscode.Diagnostic[] = [];
  const text = document.getText();
...
function provideTemplateHover(
  ...
  if (
    ...
    return new vscode.Hover(
      "**Template Logic**: Control flow directive",
      range,
    );
  }
...
function provideTemplatePlaceholderCompletions(
  ...
  const ifSnippet = new vscode.CompletionItem(
    "if",
  ...
  const ifElseSnippet = new vscode.CompletionItem(
    "ifelse",
    vscode.CompletionItemKind.Snippet,
  );
  ...
  const placeholderDescriptions: Record<string, string> = {
    ...
    // Fields & data
    FIELDS: "Constructor parameters (required this.fieldName)",
    FIELD_DECLARATIONS: "Field declarations (final Type fieldName)",
    FIELD_NAMES: "Comma-separated field names",
    FIELDS_WITH_OPTIONAL: "Optional copyWith parameters (Type? fieldName)",
...
function normalizeTemplateSyntax(text: string): string {
  ...
  const processedLines = lines.map((line) => {
    ...
    const controlOnlyMatch = line.match(
      /^\s*(\{\{(?:if\s+[a-zA-Z0-9_]+|else\s+if\s+[a-zA-Z0-9_]+|else|endif)\}\})\s*$/,
    );
    if (controlOnlyMatch) {
...
function tryFullFileFormat(
  ...
  try {
    ...
    const hasAllMarkers = controlLines.every((ctrl) =>
      formattedText.includes(ctrl.marker),
    );
    if (!hasAllMarkers) {
...
function updateDecorations(editor: vscode.TextEditor): void {
  ...
  while ((match = templateRegex.exec(text)) !== null) {
    ...
    const closeBracketStart = editor.document.positionAt(
    ...
    while ((varMatch = varRegex.exec(content)) !== null) {
      // Skip keywords
      if (
        ["if", "else", "endif", "true", "false", "null"].includes(varMatch[1])
      ) {
        continue;
      }
